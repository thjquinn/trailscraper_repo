...
```{python}
links = pd.read_csv('indiana_links.csv')

links['numbers'] = links['link'].str.extract('([0-9]{7}.*)')

indiana_list = links['numbers'].tolist()
```
...
```{python}
indiana_trail_df = pd.concat(empty_list).reset_index()
```


```{python}
indiana_trail_df = indiana_trail_df.drop(columns=['index'])
```


```{python}
indiana_trail_df.to_csv('indiana_trails.csv', index=False)
```