...
```{python}
links = pd.read_csv('pennsylvania_links.csv')

links['numbers'] = links['link'].str.extract('([0-9]{7}.*)')

pennsylvania_list = links['numbers'].tolist()
```
...
```{python}
pennsylvania_trail_df = pd.concat(empty_list).reset_index()
```


```{python}
pennsylvania_trail_df = pennsylvania_trail_df.drop(columns=['index'])
```



```{python}
pennsylvania_trail_df.to_csv('pennsylvania_trails.csv', index=False)
```